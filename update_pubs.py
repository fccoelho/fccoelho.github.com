...
def save_bib(refs):
    ...
    """
    refs_sorted = sorted(
        [r for r in refs if "pub_year" in r], key=lambda x: x["pub_year"], reverse=True
    )
    if len(refs) != len(refs_sorted):
        refs_missing = [r for r in refs if r not in refs_sorted]
        print(
            f"Missing {len(refs_missing)} references in pub list, due to missing pub_year field. Adding them to the end."
        )
